...
class Le{
    ...
    static async #req(url, payload= undefined, key = null){
        let opt = {};
        if(window.parseSearch?.().proxy?.[0]==="true")opt.mode = "cors";
    ...
    static async makeOrder(key,prof,cb = ()=>{}){
        ...
        await cb(-1,['text', 'key loaded'],['obj',{key}]);
        ...
        await cb(-1,['text', 'csr generated'],['obj',{csr}]);
        await cb(-1,['text', 'create order']);
        this.#nonceStorage=[];
        ...
        await cb(-1,['text', 'parse order'],['obj',{order}]);
                        ...
                        .then(async r => {
                            if (r.status) {
                                ...
                                if (dns01.status === 'pending') {
                                    ...
                                    await cb(r.index, ['text', 'remove authKey dns old record'])
                                    await userStore.data.dns.removeRecords('_acme-challenge.' + r.identifier.value, 'TXT');
                                    await cb(r.index, ['text', 'set new authKey dns record'])
                                    await userStore.data.dns.addRecord('_acme-challenge.' + r.identifier.value, 'TXT', dns01.print);
                                    await cb(r.index, ['text', 'wait dns update'], ['obj', {
                                        idn: '_acme-challenge.' + r.identifier.value,
                                        value: dns01.print
                                    }])
                                    for (let s = 20; s >= 0; s--) {
                                        await cb(r.index, ['sleep', s]);
                                        if (s < 1) break;
                                        await delay(1000);
                                    }
                                    await cb(r.index, ['text', 'authKey request'], ['url', dns01.url])
                                    let st = await this.#req(dns01.url, {
                                        keyAuthorization: authKey
                                    })
                                        .then(r => r[0]);
                                    while (true) {
                                        if (st.status === 'pending') {
                                            await cb(r.index, ['text', 'pending'], ['obj', {auth: st}])
                                            await delay(5000);
                                            st = await this.#req(dns01.url)
                                                .then(r => r[0]);
                                        } else break;
                                        await cb(r.index, ['status', st.status], ['obj', {auth: st}])
                                    }
                                    return st.status;
                                }
                                return dns01.status;
                            } else {
                                await cb(r.index, ['error', r.error]);
                                return null;
                            }
                        })
                    ) ?? []
        ...
        if(order['status']==='ready') {
            await cb(-1, ['text', 'finalize'], ['url', order['finalize']]);
            let fin = await this.#req(order['finalize'], {csr: csr});
            let retry = (fin[1]?.headers?.get('retry-after') ?? 3) * 1000;
            while (order.status !== 'valid') {
                await cb(-1, ['text', 'cert pending'], ['obj', order]);
                await delay(retry);
        ...
        if(order['status']==='valid') {
            await cb(-1, ['text', 'check cert'], ['obj', order]);
            if (!order['certificate']) return false;
            let cert = await this.#req(order['certificate'])
                .then(r => r[0]);
            await cb(-1, ['text', 'CERT OK'], ['cert', cert.text]);
