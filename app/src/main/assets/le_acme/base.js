    ...
    if(userStore.states.privates){
        for (const pin in userStore.states.privates) {
            if (Object.hasOwn(userStore.states.privates, pin)) {
                await Asn1.parse(userStore.states.privates[pin])
                    .then(r=>r.encode("B64","PRIVATE KEY"))
                    .then(loadData)
                    .then(r=>fixPrivate(r[0]))
...
async function packer(pack, data){
    async function packKey(val){
        ...
        if(Number(key.c(0).b)){
            ...
            return Uint8Array.from([1,3,4,5,6,7,8].map(q=>{
...
`, "Set User Email",true);
            ...
            case userStore.id.orderCreate.id:{
                let key_options = Object.keys(userStore.states.privates??{})
                    .map(v=>`<option value="${v}" title="${userStore.id.privateList.querySelector('div[data-hash="'+v+'"] small.keyInfo').innerText}">${v}</option>`).join('');
...
async function makeOrder(key,prof){
    ...
    try {
        if(!await Le.makeOrder(key, prof, async (ch, ...ar) => {
            log(ch, ...ar);
            if(ch===-1){
                switch(ar[0]?.[0]){
                    ...
                    case 'text':{
                        switch(ar[0]?.[1]){
                            default:throw "unknown text "+ar[0]?.[1];
                            case 'check cert':
                            case 'cert pending':
                            case 'finalize':
                            case 'check auths':
                ...
                let block = oWin.querySelector(`:scope>div.list-group-item[data-channel="${ch}"]`)
                if(block === null){
                    block = document.createElement('div');
                    block.className="list-group-item list-group-item-action flex-column align-items-start bg-gradient";
                    block.dataset.channel = ch.toString();
                    oWin.append(block);
                }
                switch(ar[0]?.[0]) {
                    default:print(block,"->unknown " + ar[0]?.[0]);throw "->unknown " + ar[0]?.[0];
                    case 'text':{
                        switch(ar[0]?.[1]){
                            default:print(block,"->unknown text " + ar[0]?.[1]);throw "->unknown text "+ar[0]?.[1];
                            case 'init':
                                if(ar[1][1].status==='valid'){
                                    oWin.removeChild(block);
                                    break;
                                }
                                print(block,`${ar[0][1]}: ${ar[1][1]['wildcard']?'*.':''}${ar[1][1].identifier?.value??'#####'}`);
                                break;
                            case 'pending':
                            ...
                            case 'remove authKey dns old record':
                            case 'prepare authKey':
                                print(block,`${ar[0][1]}`);
                                break;
                        }
                        break;
                    ...
                    case 'status':{
                        print(block,`Status: ${ar[0][1]}`);
                        break;
                    }
                    case 'sleep':{
                        print(block,`Sleep: ${ar[0][1]}`,true);
                        break;
                    }
                    case 'error':{
                        print(block,`ER: ${JSON.stringify(ar[0][1])
                        }`,true);
                        break;
                    }
                }
